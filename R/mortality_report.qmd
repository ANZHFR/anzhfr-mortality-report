...
# Set default figure sizes globally here to keep chunks clean
fig-height: 10
fig-width: 10
---
...
# Source plotting functions needed for the report
# Note: In a strict targets workflow, we usually avoid sourcing files in reports,
# but we need 'replace_funnel_hname' available in the environment.
tar_source(here("R/mod_functions.R"))
...
latest_report_year <- max(dat$report_year, na.rm = TRUE)
prev_year <- latest_report_year - 1
start_3yr <- latest_report_year - 2

# Helper Function to load targets dynamically
# This prevents hardcoding "2025" or "au" in chunk code.
# Assumes target naming convention: {base}_{outcome}_{year}_{country}
get_plot_target <- function(base_name, outcome, year, country) {
  target_name <- paste(base_name, outcome, year, country, sep = "_")
  # Use tar_read to read a target using a string name
  tar_read_raw(target_name)
}

# Pre-calculate counts for text
hosp_counts <- list(
  total = n_distinct(dat$h_name[dat$report_year == latest_report_year]),
  au = n_distinct(dat$h_name[dat$report_year == latest_report_year & dat$country == "au"]),
  nz = n_distinct(dat$h_name[dat$report_year == latest_report_year & dat$country == "nz"])
)

# Pre-calculate ASA Missingness
calc_asa_miss <- function(data, years) {
  data |>
    filter(report_year %in% years) |>
    group_by(is_missing = is.na(asa)) |>
    summarise(n = n(), .groups = "drop") |>
    mutate(pct = percent(n / sum(n), accuracy = 0.1)) |>
    mutate(n_fmt = comma(n)) |>
    filter(is_missing)
}

asa_this <- calc_asa_miss(dat, start_3yr:latest_report_year)
asa_last <- calc_asa_miss(dat, (start_3yr - 1):prev_year)
```
...
The Australian and New Zealand Hip Fracture Registry (ANZHFR) extends its sincere thanks to the multidisciplinary teams of the `r hosp_counts$total` hospitals that contributed patient-level data in `r latest_report_year` (`r hosp_counts$au` in Australia and `r hosp_counts$nz` in New Zealand).  Your extraordinary efforts make this report possible.
...
## INTRODUCTION
Mortality has been risk-adjusted for age, sex, pre-morbid level of function (mobility), fracture type, residence type and ASA grade. Data is presented for two follow-up periods and in two ways. The follow-up periods are 30- and 365-days following presentation with a hip fracture. Both 30-day and 365-day mortality are common benchmarks for hip fracture care. Mortality at 365 days is more likely to be influenced by factors beyond hospital care, but remains an important outcome for patients. 

ASA grade has been aggregated as (i) ASA grades 1 to 2; (ii) ASA grade 3; and (iii) ASA grades 4 and 5, according to relevant literature[^1]. 

It is important to note that ASA grade was missing or recorded as unknown in `r asa_this$n_fmt` (`r asa_this$pct`) of patient records in `r start_3yr`-`r latest_report_year` and `r asa_last$n_fmt` (`r asa_last$pct`) of patient records in `r (start_3yr - 1)`-`r prev_year`. The proportion of missings/unknowns affects standardising mortality at the hospital level. Reviewing, and where needed, increasing the proportion of patients for whom a known ASA grade is recorded as part of the data collection should be an area of continuous improvement for hospitals. 

Usual place of residence has been aggregated into two categories (i) private residence or (ii) residential care facility or other. There were a very small number of missing and unknown in covariates other than ASA that were affecting the statistical model. The missing/unknown values of covariates were handled by multiple imputation by chained equation (MICE)[^2] before constructing the statistical model. 
...
#| fig-cap: !expr paste0("Standardised mortality rate at 365 days by year for Australian states and New Zealand (2016-",prev_year,")") 
...
#| fig-cap: !expr paste0("Funnel plot of standardised mortality rate at 30 days – New Zealand hospitals (", start_3yr, "-", latest_report_year, ")") 

p <- get_plot_target("mi_funnel", "mort30d", latest_report_year, "nz") + 
  theme(plot.title = element_blank()) + 
  labs(y = "Standardised mortality rate within 30 days")
replace_funnel_hname(p, hoscode)
```

```{r}
#| label: fig-mort30d-nz-ctpl
#| fig-cap: !expr paste0("Caterpillar plot of standardised mortality rate at 30 days – New Zealand hospitals (", start_3yr, "-", latest_report_year, ")") 

p <- get_plot_target("mi_smort_ctpl", "mort30d", latest_report_year, "nz") + 
  theme(plot.title = element_blank()) + 
  labs(y = "Standardised mortality rate within 30 days")
...
#| fig-cap: !expr paste0("Funnel plot of standardised mortality rate at 365 days – New Zealand hospitals (", start_3yr - 1, "-", prev_year, ")") 

# NOTE: 365d reports use the previous year
p <- get_plot_target("mi_funnel", "mort365d", latest_report_year, "nz") + 
  theme(plot.title = element_blank()) + 
  labs(y = "Standardised mortality rate within 365 days")
replace_funnel_hname(p, hoscode)
```

```{r}
#| label: fig-mort365d-nz-ctpl
#| fig-cap: !expr paste0("Caterpillar plot of standardised mortality rate at 365 days – New Zealand hospitals (", start_3yr - 1, "-", prev_year, ")")

# NOTE: 365d reports use the previous year
p <- get_plot_target("mi_smort_ctpl", "mort365d", latest_report_year, "nz") + 
  theme(plot.title = element_blank()) + 
  labs(y = "Standardised mortality rate within 365 days")
replace_ctpl_hname(p , hoscode)
```
...
#| fig-cap: !expr paste0("Funnel plot of standardised mortality rate at 3O days – Australian hospitals (", start_3yr, "-", latest_report_year, ")") 

p <- get_plot_target("mi_funnel", "mort30d", latest_report_year, "au") + 
  theme(plot.title = element_blank()) + 
  labs(y = "Standardised mortality rate within 30 days")
replace_funnel_hname(p, hoscode)
```
...
#| fig-cap: !expr paste0("Caterpillar plot of standardised mortality rate at 30 days – Australian hospitals (", start_3yr, "-", latest_report_year, ")")

p <- get_plot_target("mi_smort_ctpl", "mort30d", latest_report_year, "au") + 
  theme(plot.title = element_blank()) + 
  labs(y = "Standardised mortality rate within 30 days")
replace_ctpl_hname(p , hoscode)
```
...
#| fig-cap: !expr paste0("Funnel plot of standardised mortality rate at 365 days – Australian hospitals (", start_3yr - 1, "-", prev_year, ")") 

# NOTE: 365d reports use the previous year
p <- get_plot_target("mi_funnel", "mort365d", latest_report_year, "au") + 
  theme(plot.title = element_blank()) + 
  labs(y = "Standardised mortality rate within 365 days")
replace_funnel_hname(p, hoscode)
```
...
#| fig-cap: !expr paste0("Caterpillar plot of standardised mortality rate at 365 day – Australian hospitals (", start_3yr - 1, "-", prev_year, ")") 

# NOTE: 365d reports use the previous year
p <- get_plot_target("mi_smort_ctpl", "mort365d", latest_report_year, "au") + 
  theme(plot.title = element_blank()) + 
  labs(y = "Standardised mortality rate within 365 days")
replace_ctpl_hname(p , hoscode)
```
